...
const AddStudentForm = ({ onAdd, onClose }) => {
    const [form, setForm] = useState({ name: "", email: "", phone: "", course: "", year: "", cgpa: "" });
    const f = (k) => (e) => setForm(p => ({ ...p, [k]: e.target.value }));

    const handleSubmit = (e) => {
        e.preventDefault();
        onAdd({ ...form, id: Date.now(), status: "Active", avatar: form.name.split(" ").map(w => w[0]).join("").slice(0, 2).toUpperCase(), joined: "Now" });
        onClose();
    };
    ...
    return (
        <form onSubmit={handleSubmit} className="space-y-4">
            <div className="grid grid-cols-2 gap-4">
                <div className="col-span-2"><Input label="Full Name" value={form.name} onChange={f("name")} placeholder="e.g. Arjun Menon" required /></div>
                <Input label="Email Address" type="email" value={form.email} onChange={f("email")} placeholder="student@example.com" required />
                <Input label="Phone Number" value={form.phone} onChange={f("phone")} placeholder="9876543210" required />
                <Select label="Course" value={form.course} onChange={f("course")} options={["B.Tech CSE", "B.Tech ECE", "B.Tech EEE", "BCA", "MCA", "MBA", "M.Tech", "B.Com", "B.Sc"]} required />
                <Select label="Year" value={form.year} onChange={f("year")} options={["1st Year", "2nd Year", "3rd Year", "Final Year"]} required />
                <div className="col-span-2"><Input label="CGPA" type="number" value={form.cgpa} onChange={f("cgpa")} placeholder="e.g. 8.5" /></div>
            </div>
...
const navItems = [
    ...
    { key: "companies", label: "Companies", icon: "M19 21V5a2 2 0 00-2-2H7a2 2 0 00-2 2v16m14 0h2m-2 0h-5m-9 0H3m2 0h5M9 7h1m-1 4h1m4-4h1m-1 4h1m-5 10v-5a1 1 0 011-1h2a1 1 0 011 1v5m-4 0h4" },
    { key: "job-requests", label: "Job Requests", icon: "M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2", badge: true },
...
const Sidebar = ({ active, setActive, sidebarOpen, setSidebarOpen, pendingCount }) => (
    <>
        {/* Mobile overlay */}
        {sidebarOpen && <div className="fixed inset-0 z-30 lg:hidden" style={{ background: "rgba(7,30,61,0.5)" }} onClick={() => setSidebarOpen(false)} />}

        <aside className={`fixed top-0 left-0 h-full z-40 flex flex-col transition-transform duration-300 ${sidebarOpen ? "translate-x-0" : "-translate-x-full"} lg:translate-x-0 lg:static lg:z-auto`}
            style={{ width: 240, background: `linear-gradient(180deg,${C.mid} 0%,${C.dark} 50%,${C.darkest} 100%)`, boxShadow: "4px 0 24px rgba(7,30,61,0.3)" }}>
            ...
            {/* Logo */}
            <div className="relative flex items-center gap-3 px-6 py-5 border-b" style={{ borderColor: "rgba(255,255,255,0.1)" }}>
                ...
                <div>
                    <span className="text-white font-extrabold text-lg tracking-wide">Job<span style={{ color: C.lighter }}>Pro</span></span>
                    <p className="text-blue-300 text-xs font-semibold">Admin Panel</p>
                </div>
                <button className="lg:hidden ml-auto text-blue-300 hover:text-white" onClick={() => setSidebarOpen(false)}>
            ...
            <nav className="relative flex-1 px-3 py-4 space-y-1 overflow-y-auto">
                <p className="px-3 text-xs font-bold uppercase tracking-widest mb-3" style={{ color: "rgba(255,255,255,0.3)" }}>Main Menu</p>
                {navItems.map(({ key, label, icon, badge }) => {
                    const isActive = active === key;
                    return (
                        <button key={key} onClick={() => { setActive(key); setSidebarOpen(false); }}
                            className="w-full flex items-center gap-3 px-4 py-3 rounded-xl text-sm font-semibold text-left transition-all duration-200 group"
                            style={{
                                background: isActive ? C.sidebarActive : "transparent",
                                color: isActive ? "#fff" : "rgba(255,255,255,0.6)",
                                boxShadow: isActive ? `0 4px 16px rgba(21,101,192,0.4)` : "none",
                            }}
                            onMouseEnter={e => { if (!isActive) e.currentTarget.style.background = "rgba(255,255,255,0.08)"; }}
                            onMouseLeave={e => { if (!isActive) e.currentTarget.style.background = "transparent"; }}>
                            <svg className="w-4.5 h-4.5 flex-shrink-0" style={{ width: 18, height: 18 }} fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24">
                                {icon.split(" M").map((d, i) => <path key={i} d={i === 0 ? d : "M" + d} />)}
                            </svg>
                            <span className="flex-1">{label}</span>
                            {badge && pendingCount > 0 && (
                                <span className="text-xs px-2 py-0.5 rounded-full font-black"
                                    style={{ background: "#f59e0b", color: "#fff" }}>{pendingCount}</span>
                            )}
                            {isActive && <span className="w-1.5 h-1.5 rounded-full" style={{ background: C.lighter }} />}
                        </button>
                    );
                })}
            </nav>
            ...
            <div className="relative p-4 border-t" style={{ borderColor: "rgba(255,255,255,0.1)" }}>
                <div className="flex items-center gap-3 p-3 rounded-xl" style={{ background: "rgba(255,255,255,0.07)" }}>
                    ...
                    <div className="flex-1 min-w-0">
                        ...
                        <p className="text-blue-300 text-xs truncate">admin@jobpro.com</p>
                    </div>
...
const SettingsPage = ({ showToast }) => {
    const [profile, setProfile] = useState({ name: "Super Admin", email: "admin@jobpro.com", phone: "+91 98765 43210" });
    const f = k => e => setProfile(p => ({ ...p, [k]: e.target.value }));
    return (
        <>
            <TopBar title="Settings" subtitle="Manage your admin preferences" setSidebarOpen={() => { }} />
            <div className="p-6 max-w-2xl space-y-6">
                <div className="p-6 rounded-2xl" style={{ background: "#fff", border: "1px solid #e2e8f0" }}>
                    <h3 className="font-extrabold text-gray-900 mb-5">Admin Profile</h3>
                    <div className="flex items-center gap-4 mb-6">
                        <div className="w-16 h-16 rounded-2xl flex items-center justify-center text-white font-black text-xl"
                            style={{ background: `linear-gradient(135deg,${C.light},${C.main})` }}>SA</div>
                        <div>
                            <p className="font-extrabold text-gray-900">{profile.name}</p>
                            <p className="text-sm text-gray-500">{profile.email}</p>
                        </div>
                    </div>
                    <div className="space-y-4">
                        <Input label="Full Name" value={profile.name} onChange={f("name")} />
                        <Input label="Email" type="email" value={profile.email} onChange={f("email")} />
                        <Input label="Phone" value={profile.phone} onChange={f("phone")} />
                    </div>
                    <button onClick={() => showToast("Profile updated!", "success")}
                        ...
                        style={{ background: `linear-gradient(135deg,${C.main},${C.mid})`, boxShadow: `0 4px 16px rgba(21,101,192,0.3)` }}>
                        Save Changes
                    </button>
                </div>

                <div className="p-6 rounded-2xl" style={{ background: "#fff", border: "1px solid #e2e8f0" }}>
                    <h3 className="font-extrabold text-gray-900 mb-1">Change Password</h3>
                    <p className="text-gray-500 text-sm mb-5">Use a strong password with at least 8 characters.</p>
                    <div className="space-y-4">
                        <Input label="Current Password" type="password" placeholder="••••••••" />
                        <Input label="New Password" type="password" placeholder="••••••••" />
                        <Input label="Confirm New Password" type="password" placeholder="••••••••" />
                    </div>
                    <button onClick={() => showToast("Password changed!", "success")}
                        className="mt-5 px-6 py-2.5 rounded-xl text-sm font-bold text-white transition-all hover:-translate-y-0.5"
                        style={{ background: `linear-gradient(135deg,${C.main},${C.mid})`, boxShadow: `0 4px 16px rgba(21,101,192,0.3)` }}>
                        Update Password
                    </button>
